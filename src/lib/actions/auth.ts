...
function getVerificationSecret() {
  return (
    ...
    'qingchuang-verification-secret'
  )
}
...
async function verifyTurnstileToken(token: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    if (!data.success) {
      ...
      return { success: false, error: '安全验证失败，请刷新页面后重试。' }
    }

    return { success: true }
  } catch (error) {
    console.error('Turnstile verification error:', error)
    return { success: false, error: '安全验证服务暂时不可用，请稍后再试。' }
  }
}
...
function buildRegisterCodeEmail(code: string) {
  ...
  return {
    subject: '轻创注册验证码',
    text: `你的轻创注册验证码是 ${code}，10 分钟内有效。如非本人操作，请忽略这封邮件。`,
    html: `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f8fafc;padding:32px;color:#0f172a;">
        <div style="max-width:560px;margin:0 auto;">
          ...
          <div style="background:#ffffff;border-radius:24px;padding:32px;border:1px solid #e2e8f0;box-shadow:0 20px 60px rgba(15,23,42,0.08);">
            ...
            <div style="font-size:16px;line-height:1.9;color:#475569;margin-bottom:24px;">
              你正在注册轻创账号，请输入下面的 6 位验证码完成验证。验证码 10 分钟内有效。
            </div>
            <div style="font-size:40px;letter-spacing:12px;font-weight:800;color:#047857;background:#ecfdf5;border:1px solid #a7f3d0;border-radius:20px;padding:20px 24px;text-align:center;margin-bottom:20px;">
              ${code}
            </div>
            <div style="font-size:14px;line-height:1.9;color:#64748b;">
              如非本人操作，请直接忽略这封邮件。<br />
              访问轻创：<a href="${appUrl}" style="color:#047857;text-decoration:none;">${appUrl}</a>
            </div>
          </div>
...
async function sendBrevoEmail(input: {
  ...
  if (!senderEmail) {
    throw new Error('缺少发件邮箱，请在环境变量中配置 BREVO_SENDER_EMAIL。')
  }
...
async function storeRegisterVerificationCode(email: string, code: string) {
  ...
  if (
    existing?.sent_at &&
    now - new Date(existing.sent_at).getTime() < REGISTER_CODE_RESEND_COOLDOWN_MS
  ) {
...
async function verifyRegisterCodeAndCreateUser(input: {
  ...
  if (input.scope === 'admin') {
    return { error: '管理员账号不支持通过邮箱验证码自行注册。' }
  }
...
export async function requestEmailCode(formData: FormData) {
  ...
  if (!turnstileResult.success) {
    return { error: turnstileResult.error || '安全验证失败，请重试。' }
  }

  if (mode !== 'register') {
    return { error: '登录请直接使用邮箱和密码。' }
  }
  ...
  try {
    ...
    try {
      ...
      await removeRegisterVerificationRecord(email).catch(() => undefined)
      throw error
    }
    ...
    return {
      error: error instanceof Error ? error.message : '发送注册验证码失败，请稍后再试。',
    }
  }
}
...
export async function verifyEmailCode(formData: FormData) {
  ...
  try {
    ...
    return {
      error: error instanceof Error ? error.message : '注册验证失败，请稍后再试。',
    }
  }
}
...
export async function loginWithPassword(formData: FormData) {
  ...
  return {
    success: true,
    redirectTo: role === 'admin' && scope === 'admin' ? '/admin' : '/campus',
  }
}
